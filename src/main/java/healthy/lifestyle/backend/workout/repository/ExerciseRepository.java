...
public interface ExerciseRepository extends JpaRepository<Exercise, Long> {
    ...
    @Query("SELECT e FROM Exercise e WHERE (e.title = :title AND e.isCustom = true AND e.user.id = :userId) "
            + "OR (e.title = :title AND e.isCustom = false)")
    List<Exercise> findDefaultAndCustomByTitleAndUserId(String title, Long userId);
